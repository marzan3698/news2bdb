...
<div class="row pt-4">
    <div class="col-sm-12">
        <div class="page-title-box">
            ...
            <h4 class="page-title">Edit AI Source</h4>
        </div>
    </div>
</div>
...
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                <h4 class="header-title mt-0 mb-4">Edit AI News Source Config</h4>
                ...
                <form action="{{ route('admin.ai-sources.update', $source->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="form-group mb-3">
                        <label for="name" class="font-weight-bold">Source Name</label>
                        <input type="text" name="name" class="form-control" id="name" placeholder="e.g. Prothom Alo RSS" value="{{ old('name', $source->name) }}" required>
                    </div>

                    <div class="form-group mb-3">
                        <label for="type" class="font-weight-bold">Source Type</label>
                        <select name="type" id="type" class="form-control" required>
                            <option value="rss" {{ old('type', $source->type) == 'rss' ? 'selected' : '' }}>RSS Feed Link (Recommended)</option>
                            <option value="facebook" {{ old('type', $source->type) == 'facebook' ? 'selected' : '' }}>Facebook Page ID/Username</option>
                            <option value="scraping" {{ old('type', $source->type) == 'scraping' ? 'selected' : '' }}>Web Page HTML Scraping</option>
                        </select>
                    </div>

                    <div class="form-group mb-3">
                        <label for="url" class="font-weight-bold">URL / Address / Page identifier</label>
                        <input type="text" name="url" class="form-control" id="url" placeholder="e.g. https://www.prothomalo.com/feed or facebook-page-username" value="{{ old('url', $source->url) }}" required>
                        <small class="form-text text-muted" id="url-help">Provide the full RSS XML feed URL.</small>
                    </div>

                    <div class="form-group mb-3">
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="status" name="status" value="1" {{ $source->status ? 'checked' : '' }}>
                            <label class="custom-control-label font-weight-bold" for="status">Active & Enabled</label>
                        </div>
                    </div>

                    <button type="submit" class="btn btn-primary mt-3"><i class="mdi mdi-content-save"></i> Save Changes</button>
